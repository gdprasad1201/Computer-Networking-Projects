...
enum SenderState { WAIT_LAYER5, WAIT_ACK };
...
void A_output(struct msg message) {
    if (SenderA.state != WAIT_LAYER5) {
        printf("A_output: not acked...drop the message: %s\n", message.data);
        return;
    }
    ...
    SenderA.last_packet = packet;
    SenderA.state = WAIT_ACK;
...
void A_input(struct pkt packet) {
    if (SenderA.state != WAIT_ACK) {
        printf("A_input: not waiting ACK...drop.\n");
        return;
    }
    ...
    SenderA.seq = 1 - SenderA.seq;
    SenderA.state = WAIT_LAYER5;
}
...
void A_timerinterrupt() {
    ...
    printf("A_timerinterrupt: resend last packet: %s.\n",
           SenderA.last_packet.payload);

    tolayer3(0, SenderA.last_packet);
    starttimer(0, SenderA.estimated_rtt);
}
...
void B_input(struct pkt packet) {
    if (packet.checksum != get_checksum(&packet)) {
        ...
        send_ack(1, 1 - ReceiverB.seq);

        return;
    }

    if (packet.seqnum != ReceiverB.seq) {
    ...
    send_ack(1, ReceiverB.seq);
    tolayer5(1, packet.payload);

    ReceiverB.seq = 1 - ReceiverB.seq;
}
